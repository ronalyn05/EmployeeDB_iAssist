...
const NewHireUpload = () => {
  ...
  //     if (invalidRoles.length > 0) {
  //       const invalidRoleRows = invalidRoles.map(
  //         (row, index) => `Row ${index + 1}`
  //       );
  //       throw new Error(
  //         `Invalid Role values detected in the following rows: ${invalidRoleRows.join(
  //           ", "
  //         )}. Please ensure the Role is either 'HRAdmin' or 'Employee'.`
  //       );
  //     }
  ...
  //     alert(
  //       "Data has been successfully uploaded and Email sent successfully to each account user!"
  //     );
  //     console.log("Upload response:", response.data);
